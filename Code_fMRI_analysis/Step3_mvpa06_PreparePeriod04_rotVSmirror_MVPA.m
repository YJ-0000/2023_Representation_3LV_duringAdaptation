...
cd ..
mkdir('MVPA_PreparePeriod_rotationVSmirror'); cd('MVPA_PreparePeriod_rotationVSmirror');
pathMVPA = pwd;
...
parfor nsub = 1:length(results1st_subj)
    ...
    labelnames = {'session_start_rotation+90','session_start_rotation-90','session_start_mirror'};
...
%     cfg.design.test = cfg.design.test(:,1:end-1);
    cfg.design.train = repmat(cfg.design.train,[1,2]);
    cfg.design.test = repmat(cfg.design.test,[1,2]);
    cfg.design.train(1:5,1:5) = 0;
    cfg.design.test(1:5,1:5) = 0;
    cfg.design.train(6:10,6:10) = 0;
    cfg.design.test(6:10,6:10) = 0;
    cfg.design.set = ones(1,10);
    cfg.design.label = repmat(cfg.design.label,[1,2]);
