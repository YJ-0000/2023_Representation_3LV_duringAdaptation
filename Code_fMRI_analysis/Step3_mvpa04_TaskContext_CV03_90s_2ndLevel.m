...
cd ..
cd('MVPA_TaskContext_CV_posneg90s');
pathMVPA = pwd;
mvpa_1st_subj = dir('sub-*');

cd ..
mkdir('GroupLevel_MVPA_TaskContext_CV_posneg90s'); cd('GroupLevel_MVPA_TaskContext_CV_posneg90s');
targetPath = pwd;
...
matlabbatch{1}.spm.stats.factorial_design.masking.em = {[pathSPMmvpa '\GMmask.nii,1']};
