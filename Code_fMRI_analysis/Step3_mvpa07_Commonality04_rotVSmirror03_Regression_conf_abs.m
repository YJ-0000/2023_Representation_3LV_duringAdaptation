...
cd ..
mkdir('GroupLevel_MVPA_TaskContext_rotationVSmirror_regression_withConfounds'); cd('GroupLevel_MVPA_TaskContext_rotationVSmirror_regression_withConfounds');
targetPath = pwd;
...
%     mean_perf_eachsub(nsub,1) = mean(performance_eachsub(nsub,nday,:,:),'all',"omitnan");
    mean_perf_eachsub_rots(nsub,1) = (mean(performance_eachsub(nsub,nday,squeeze(session_typeidx_eachsub(nsub,nday,:)==1),:),'all',"omitnan") ...
...
mean_perf_eachsub_rots(outlier_idx) = [];
...
matlabbatch{1}.spm.stats.factorial_design.des.mreg.mcov(1).c = mean_perf_eachsub_mirror;
matlabbatch{1}.spm.stats.factorial_design.des.mreg.mcov(1).cname = 'Session accuracy (mirror)';
matlabbatch{1}.spm.stats.factorial_design.des.mreg.mcov(1).iCC = 5;
matlabbatch{1}.spm.stats.factorial_design.des.mreg.mcov(2).c = abs(mean_perf_eachsub_rots - mean_perf_eachsub_mirror);
matlabbatch{1}.spm.stats.factorial_design.des.mreg.mcov(2).cname = 'absolute Session accuracy difference (rotation - mirror)';
...
matlabbatch{3}.spm.stats.con.consess{1}.tcon.name = 'result (mirror)';
